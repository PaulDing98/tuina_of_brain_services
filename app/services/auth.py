...
LastEditTime: 2026-04-18 20:07:15
...
from app.core.exceptions import UserAlreadyExistsException, AppException, InvalidCredentialsException
...
def login_user(data: LoginRequest, db: Session) -> AuthResponseData:
    ...
    if not PasswordHandler.verify(data.password, str(db_user.hashed_password)):
        raise InvalidCredentialsException()
    # 3. 生成并返回令牌
    return build_auth_response(str(db_user.id))
